...
def setup_bot_files():
    ...
    if not os.path.exists(config_file):
        initial_config = {
            ...
            "bot_status": {"type": "playing", "name": ".gg/flowcloud | $info"},
            "embed_color": "#00FFFF",
            "embed_footer_text": "Flow Cloud",
            "embed_thumbnail_url": "https://cdn.discordapp.com/attachments/1445528661661978767/1445538843670478992/IMG_4926.jpg"
        }
...
STATUS_MUST_BE = config.get('status_must_be', '.gg/flowcloud')
...
BOT_STATUS_CONFIG = config.get('bot_status', {"type": "playing", "name": ".gg/flowcloud | $info"})
EMBED_COLOR_HEX = config.get('embed_color', '#00FFFF')
BOT_FOOTER_TEXT_BASE = config.get('embed_footer_text', 'Flow Cloud')
EMBED_THUMBNAIL_URL = config.get('embed_thumbnail_url', 'https://cdn.discordapp.com/attachments/1445528661661978767/1445538843670478992/IMG_4926.jpg')
...
USER_ROLES_FILE = os.path.join(os.path.dirname(__file__), 'user_status_roles.json')
def load_user_roles():
...
async def log_status_event(guild, event_type, user, role_name=None, reason=None):
    ...
    if not log_channel:
        return
    embed = discord.Embed(
        title=f"Flow Cloud Status Log - {event_type}",
        color=EMBED_COLOR,
...
@bot.hybrid_command(name='status', help='Display bot status and system information')
...
async def botinformation_command(ctx: commands.Context):
    ...
    embed = create_enhanced_embed(
        ...
        description="Detailed information about Flow Cloud Gen Bot",
        color=discord.Color.blue(),
        ctx_or_interaction=ctx
    )
...
@bot.hybrid_command(name='send-gen-access')
async def send_gen_access(ctx: commands.Context, channel: discord.TextChannel = None):
    ...
    embed.add_field(
        ...
        value="Go into the verification channel and run `$cstatus`",
        inline=False
    )
    embed.add_field(
...
@bot.hybrid_command(name='cstatus')
...
if __name__ == "__main__":
    setup_bot_files()          # create / ensure config.json exists
    config = load_config()     # load the actual file
    BOT_TOKEN = config.get('bot_token')  # refresh the global token

    if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        print("Error: bot_token not found in config.json or still set to placeholder.")
        exit(1)
